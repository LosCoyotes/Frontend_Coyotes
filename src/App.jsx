...
function App() {
  ...
  const getArchivados = async () => {
    try {
      const res = await fetch("http://127.0.0.1:8000/zapatos/archivados");
  ...
  const saveNewZapato = async (e) => {
    ...
    try {
      if (modoEdicion) {
        ...
        const resActualizar = await fetch(`http://127.0.0.1:8000/zapatos/actualizar/${idEditando}`, {
          method: "PUT",
        ...
        const resModelo = await fetch("http://127.0.0.1:8000/modelos", {
        ...
        if (dataModelo.estado === "Éxito") {
          const resVariante = await fetch("http://127.0.0.1:8000/zapatos", {
            method: "POST", headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
  ...
  const descontinuarZapato = async (id_variante) => {
    ...
    if (confirmar) {
      try {
        ...
        const respuesta = await fetch(`http://127.0.0.1:8000/zapatos/descontinuar/${id_variante}`, {
          method: "PUT"
        });
        const datos = await respuesta.json();
